...
void codegen(AST *root) { //register will increase when we go right side of tree
    ...
    //static int loadRcd[3]={-1,-1,-1}; //record loading of identifier
    static int regIdx;
    static int memJudge;
    static int Val;
    ...
    switch(root->kind){
        case ASSIGN:
            codegen(root->rhs);
            //left only will be idntifier, won't use register; right can remain same register
        ...
        case REM:
            ...
            codegen(root->lhs);
            regIdx--;
            printf("%s r%d r%d r%d\n",ArithOP[(root->kind)-ADD],regIdx,regIdx+1,regIdx);
        ...
        case IDENTIFIER:
            ...
            if(memJudge==1)
                printf("%s [%d] r%d\n",MemOP[memJudge],memAd[(root->val)-'x'],regIdx); //store
            else
