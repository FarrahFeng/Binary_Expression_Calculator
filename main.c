...
void codegen(AST *root) {
    ...
    switch(root->kind){
        case ASSIGN:
            ...
            memJudge++;
            //regIdx++; //since will equal to the value of right EXPR(regIDx+1)
            codegen(root->lhs);
            //regIdx--;
            memJudge--;
            break;
        ...
        case POSTDEC:
            ...
            printf("%s [%d] r%d\n",MemOP[1],memAd[((root->mid)->val)-'x'],regIdx+1); //store
        ...
        case PREDEC: //operate after adding
            ...
            printf("%s [%d] r%d\n",MemOP[1],memAd[((root->mid)->val)-'x'],regIdx); //store
        ...
        case IDENTIFIER:
            ...
            //store value of right expr (regIdx+1)
            else
